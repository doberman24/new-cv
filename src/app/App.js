...
function App() {
  ...
  return (
    <div className={mainTheme}>
      <div className="relative flex w-screen h-screen justify-between bg-white duration-300 dark:bg-neutral-900 z-0">
          ...
          <ul className="flex flex-col flex-grow justify-center gap-5 pb-20">
            {Object.entries(icons).map(([key, value], count) => 
              ...
              >
                ...
                <button className={`${effectSideMenu === count ? 'bg-[#ffa500]' : page === key ? 'bg-[#ffa500]' : 'bg-neutral-200 dark:bg-neutral-800'} rounded-full duration-300 hover:duration-300 outline-none ${modal ? '-z-10' : 'z-0'}`}
                >
                  <svg className={`${effectSideMenu === count ? 'fill-white' : page === key ? 'fill-white' : ''} dark:text-white z-10 w-6 h-6 p-4 box-content duration-300 hover:duration-300`}
